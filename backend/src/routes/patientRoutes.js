...
router.put('/:id/cancel', verifyToken, async (req, res) => {
    try {
      ...
      // ✅ Safely check ownership using optional chaining and null checks
      if (
        !req.patient || 
        !req.patient._id || 
        !appointment.patient_id || 
        appointment.patient_id.toString() !== req.patient._id.toString()
      ) {
      ...
      if (doctor) {
        ...
        const dateEntry = doctor.time_slots.find(entry => entry.date === appointmentDateStr);
  
        if (dateEntry && dateEntry.slots) {
          const slotToUpdate = dateEntry.slots.find(slot => slot.time === appointment.time_slot);
          if (slotToUpdate) {
